...
async function validateApiKey() {
    try {
        ...
        if (error.message && error.message.includes('leaked')) {
            console.error("🚨 CRITICAL: Your API key has been blocked by Google!");
            console.error("📋 Reason: The API key was reported as leaked");
            console.error("🔧 Solution:");
            console.error("   1. Visit: https://makersuite.google.com/app/apikey");
            console.error("   2. Generate a new API key");
            console.error("   3. Replace GEMINI_API_KEY in the .env file");
            console.error("   4. Restart this server");
        } else if (error.message && error.message.includes('403')) {
            console.error("🚨 API access denied - key may be invalid or blocked");
        }
        
        console.error("");
        console.error("⚠️  Server will continue running but quiz generation will fail");
        console.error("⚠️  Please fix the API key issue to restore functionality");
        return false;
    }
}
...
async function generateQuizFromText(pdfText) {
    try {
        ...
        if (error.message && error.message.includes('leaked')) {
            throw new Error("API key has been blocked by Google. Please generate a new API key from https://makersuite.google.com/app/apikey and update the .env file");
        } else if (error.message && error.message.includes('403')) {
            throw new Error("API access denied. The API key may be invalid or blocked");
...
app.listen(port, async () => {
    ...
    console.log("📝 Text extraction using PDF.js");
    console.log("");
